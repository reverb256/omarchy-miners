...
- **Service management**: The plugin manages miners through `miner-control` (start/stop/restart/status). The five Linux miners are k3s Deployments (namespace `mining`) — control routes through kubectl (runtime only; config = git + ArgoCD, repo `reverb256/mining-k8s`). Non-migrated units fall back to systemd/NSSM. Targets are validated against the fleet definition before any control call. If the kubeconfig's pinned apiserver is unreachable, `miner-control` probes the other cluster endpoints (10.1.1.100 VIP → sentry → forge → nexus) and retries the action once against the first that answers — a single control-plane outage no longer kills panel control. A status read against an unreachable API reports `state: unknown`, never a fake `inactive`.
- **State vocabulary (pause vs failure)**: `active` = ready and mining; `paused` = scaled to 0 **by the user** — a deliberate stop (the panel's Pause writes it; ArgoCD carries `ignoreDifferences` on `spec.replicas` so it sticks, and it survives reboots); `inactive` = should be running but is not ready (a failure); `unknown` = no apiserver answered. `poll.py` marks each k3s miner `paused: true` from the Deployment spec and emits `state`, so the panel, the JSON consumers, and AI agents reading the fleet can tell an intentional pause from an unintended failure — a paused miner reads "Paused", a failed one "Offline".
